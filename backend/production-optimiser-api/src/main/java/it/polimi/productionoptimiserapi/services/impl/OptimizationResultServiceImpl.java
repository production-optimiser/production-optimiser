        ...
        .map(
            (result) -> {
              ...
              result
                  ...
                  .forEach(
                      (pallet) ->
                          maximumPalletsUsed.put(pallet.getDefinedPallets(), pallet.getCount()));
              result
                  .getPalletsDefinedInExcel()
                  .forEach(
                      (pallet) ->
                          palletsDefinedInExcel.put(pallet.getDefinedPallets(), pallet.getCount()));
              result
                  .getGraphs()
                  .forEach(
                      (graph) ->
                          graphs.put(graph.getType().toString(), graph.getBase64EncodedImage()));
