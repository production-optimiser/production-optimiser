...
public class OptimizationModelController {
  ...
  public ResponseEntity<OptimizationResult> invoke(
    ...
    validateInput(om, inputFile, input);
    ...
    switch (om.getInputType()) {
      case FILE:
        if (inputFile == null) {
          ...
          if (Stream.of(ALLOWED_FILE_EXTENSIONS)
              .noneMatch(
                  ext -> ext.equals(FilenameUtils.getExtension(inputFile.getOriginalFilename())))) {
      ...
      case IMAGE:
        if (inputFile == null) {
          ...
          if (Stream.of(ALLOWED_IMAGE_EXTENSIONS)
              .noneMatch(
                  ext -> ext.equals(FilenameUtils.getExtension(inputFile.getOriginalFilename())))) {
            throw new IllegalArgumentException(
                "Invalid file extension. Allowed extensions are: "
